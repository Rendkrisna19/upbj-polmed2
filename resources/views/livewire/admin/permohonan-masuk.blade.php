<div class="w-full pb-12" wire:poll.10s>
    ...
    <div wire:loading.flex wire:target="prosesPermohonan, tolakPermohonan" class="fixed inset-0 z-[150] bg-gray-900/60 backdrop-blur-sm flex items-center justify-center transition-all">
        <div class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl flex flex-col items-center max-w-sm w-full mx-4 animate-pulse border border-gray-200 dark:border-gray-700">
            ...
            <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2 text-center">Memproses Status...</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 text-center">Mohon tunggu sebentar, sistem sedang mengubah data dan mengirimkan notifikasi.</p>
        </div>
    </div>
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight flex items-center">
            ...
            @if($permohonanMasuk->total() > 0)
                <span class="ml-4 bg-red-50 text-red-600 border border-red-200 text-xs font-bold px-2.5 py-1 rounded-full shadow-sm dark:bg-red-900/30 dark:text-red-400 dark:border-red-800">
                    {{ $permohonanMasuk->total() }} Baru
                </span>
            @endif
        </h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Daftar pengajuan baru yang menunggu verifikasi dan persetujuan Anda.</p>
    </div>
            ...
            <table class="w-full text-sm text-left border-collapse">
                <thead class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-semibold">Unit / Pengaju</th>
                    ...
                    @forelse ($permohonanMasuk as $item)
                        <tr class="hover:bg-purple-50/50 dark:hover:bg-gray-800/60 transition-colors group">
                            <td class="px-6 py-4 text-gray-600 dark:text-gray-300 whitespace-nowrap">
                                ...
                                <p class="text-xs text-gray-500 mt-1 ml-6">{{ \Carbon\Carbon::parse($item->tanggal)->format('d/m/Y') }}</p>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <button wire:click="showDetail({{ $item->id }})" class="inline-flex items-center justify-center px-4 py-2 rounded-lg text-white bg-purple-600 hover:bg-purple-700 transition-colors shadow-sm font-medium text-xs" title="Review Permohonan">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    <div class="w-16 h-16 bg-green-50 dark:bg-green-900/20 rounded-full flex items-center justify-center mb-4 text-green-500">
                                        <i class="fa-solid fa-mug-hot text-2xl"></i>
                                    </div>
                                    <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-1">Keranjang Bersih!</h3>
                                    <p class="text-sm text-gray-500 max-w-sm">Belum ada permohonan baru yang masuk saat ini.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="relative w-full max-w-4xl bg-white dark:bg-gray-900 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-gray-200 dark:border-gray-800">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-1 space-y-4">
                            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-1 uppercase font-semibold tracking-wider">Unit Pengaju</p>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-1 uppercase font-semibold tracking-wider">Tanggal Surat</p>
                                <p class="font-medium text-gray-900 dark:text-white text-sm">{{ \Carbon\Carbon::parse($selectedData->tanggal)->translatedFormat('d F Y') }}</p>
                            </div>

                            <div class="bg-purple-50 dark:bg-purple-900/10 rounded-xl p-4 border border-purple-200 dark:border-purple-800/30">
                                <p class="text-xs text-purple-700 dark:text-purple-400 mb-3 uppercase font-semibold tracking-wider flex items-center justify-between">
                                    Dokumen Lampiran
                                    <span class="bg-purple-200 text-purple-800 dark:bg-purple-800 dark:text-purple-200 py-0.5 px-2 rounded-full text-[10px]">{{ $selectedData->dokumenLampirans->count() }} File</span>
                                </p>
                                
                                <div class="space-y-2">
                                    @forelse($selectedData->dokumenLampirans as $doc)
                                        <div class="flex items-center justify-between bg-white dark:bg-gray-800 p-2.5 rounded-lg border border-purple-100 dark:border-purple-800/50 shadow-sm">
                                            ...
                                            <button wire:click="openPdfPreview('{{ $doc->file_path }}')" class="shrink-0 text-[11px] font-bold px-3 py-1.5 bg-purple-100 hover:bg-purple-200 text-purple-700 dark:bg-purple-900/50 dark:hover:bg-purple-700 dark:text-purple-300 rounded-md transition-colors">
                                                Buka
                                            </button>
                                        </div>
                                    @empty
                        ...
                        <div class="lg:col-span-2">
                            <div class="mb-5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-4 rounded-xl">
                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-1 uppercase font-semibold tracking-wider">Judul Permohonan</p>
                            ...
                            <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3 uppercase tracking-wide flex items-center border-b border-gray-200 dark:border-gray-700 pb-2">
                                Rincian Kebutuhan <span class="ml-auto bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-2.5 py-0.5 rounded text-xs">{{ $selectedData->items->count() }} Item</span>
                            </h4>
                ...
                <div class="p-5 border-t border-gray-200 dark:border-gray-800 flex flex-col-reverse sm:flex-row justify-between gap-4 bg-gray-50/50 dark:bg-gray-800/30">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        " class="px-6 py-2.5 bg-red-50 hover:bg-red-600 hover:text-white text-red-600 dark:bg-red-900/20 dark:hover:bg-red-600 dark:text-red-400 text-sm font-medium rounded-lg transition-colors border border-red-200 dark:border-red-800/50 shadow-sm flex items-center justify-center">
